...
import { useProducts, useProgramsPage, useTheme } from "@/hooks/useContent";
...
export default function ProgramsPage() {
  ...
  const { data: pageData, isLoading: pageLoading } = useProgramsPage();
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm border border-primary/20 mb-4">
              ...
              <span className="text-sm font-semibold text-primary">Beauty Programs</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-serif font-bold text-foreground mb-4">
              {hero?.title || 'Nuestros Programas'}
            </h1>
            <p className="text-lg text-muted-foreground">
              {hero?.subtitle || 'Programas especializados para ayudarte a alcanzar tus objetivos'}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              programs.map((program: any) => (
                <div key={program.id} className="bg-white rounded-2xl border border-border p-6 hover:shadow-beauty transition-shadow">
                  <h3 className="text-xl font-serif font-bold text-foreground mb-2">{program.name}</h3>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    ...
                    <button className="btn-beauty text-sm py-2 px-6">Inscribirse</button>
                  </div>
                </div>
              ))
            )}
          ...
          {!isLoading && programs.length === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="text-2xl font-serif font-bold text-foreground mb-2">No hay programas disponibles</h3>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-foreground mb-4">
              {cta?.title || '¿Tienes Preguntas?'}
            </h2>
            <p className="text-lg text-muted-foreground mb-6">
              {cta?.description || 'Contáctanos para obtener más información sobre nuestros programas'}
            </p>
            <a href="#contact" className="btn-beauty">
              {cta?.buttonText || 'Más Información'}
            </a>
          </div>
        </div>
      </section>
